...
Элемент, который автоматически вводит в выбранные поля слоя значения широты и долготы в текстовом виде. Так координаты записываются во внутреннюю структуру, и все записи будут иметь координаты. Этот элемент нужен если вам нужно получить широту и долготу в таблице атрибутов.
...
Элемент, который автоматически записывает инкрементируемое значение в заданном текстовом формате. Используется для генерации порядковых номеров. В ngw кончно есть внутренние коды записей, но они не появляются в таблице атрибутов.
